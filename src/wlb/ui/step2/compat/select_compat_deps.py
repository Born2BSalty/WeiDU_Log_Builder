...
def parse_required(text: str, norm: Callable[[str], str]) -> set[str]:
    return {norm(m.group(1)) for m in _REQUIRE_COMPONENT_RE.finditer(text)}


def parse_conflicts(text: str, norm: Callable[[str], str]) -> set[str]:
    return {norm(m.group(1)) for m in _FORBID_COMPONENT_RE.finditer(text)}


def mod_is_installed_refs(text: str, norm: Callable[[str], str]) -> list[tuple[str, bool]]:
